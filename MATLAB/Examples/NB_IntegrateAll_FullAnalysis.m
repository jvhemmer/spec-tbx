...
expName = 'r-Ag4 5mVps 50 μM NB in soln FULL SPECTRUM INTEG (3rd cycle)';
...
scanRate = 0.005; % V/s
segments = 6;
cycle = 3; % which cycle will be plotted
CVlimits = [-10 6];
...
for frame = 1:size(corrIntensity, 2)
    [area(frame)] = integratePeak(ramanShift, corrIntensity(:,frame), [min(ramanShift) max(ramanShift)]);
end
